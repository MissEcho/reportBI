...
export default class index extends PureComponent {
  ...
  static getDerivedStateFromProps(props, state) {
    const { numbers, type } = props;
    if (numbers !== state.numbers) {
      let newNumber;
      let oldNumber;
      if (type == 'percent') {
        newNumber = numbers.toString().split('');
        oldNumber = state.numbers ? state.numbers.toString().split('') : newNumber;
      } else {
        newNumber = toThousands(numbers);
        oldNumber = toThousands(state.numbers ? state.numbers : numbers);
      }
  ...
  render() {
    ...
    return (
      <ul className={styles.numberBox} style={style}>
        ...
        {
          type == 'percent' ? <li className={styles.numberItem}>%</li> : null
        }
      </ul>
    );
  }
